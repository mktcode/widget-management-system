...
try {
    $pageNameEx = explode('facebook.com/', $this->getData('url', $contentId));
    $pageName = explode('/', $pageNameEx[1]);

    $res = $fb->get(
        '/' . $pageName[0] . '/events?fields=cover,attending_count,declined_count,end_time,description,maybe_count,name,place,start_time.order(chronological)&limit=' . $this->getData(
            'events',
            $contentId
        )
    );
